/* Fichier de gestion du comportement des ROBOTS EVOLUES
   - Deplacements
   - Comportements suivant les collisions

   Vue de la vidéo : (-0.7500,0.6331,3.5700,-38.5000,-26.5000,0.0000)
*/
...
void step(Agent *agy){
	...
	case 2 :
		...
		if( ordre_arrivee == 9 ){
                        ...
                        //Est ce que quelqu'un me tient ???
			bobo = dJointGetBody( agy->arriere, 1);

			//si c'est non
			if( bobo == (dBodyID ) 0 && agy->au_dessus == 0 ) {
                                //je trouve mon suivant
				a = cherche_agent( agy->bodyID_but );
				//si il n'y a personne sur lui
				if( a != NULL && a->au_dessus == 0) {
					//personne ne me tient alors je peux essayer de monter sur le suivant

                                        //plus personne devant moi !! ;)
					agy->au_devant = 0;

					//je me libère les pattes
					dJointAttach( agy->patte, 0, 0 );

					//je le lache
					dJointAttach( agy->avant, 0, 0 );
	...
	case 3 :
		//Description : Dans cet état l'agent regarde si on l'accroche. Si non alors il monte sur celui
		// qui se trouve devant lui, parcours la chaine et s'accroche en fin
			...
			//ici on traite le cas ou un suivant existe
					...
					//printf("Cas spécial !!!!\nl'agent sur lequel je veux grimper est en bleu\n");
					b->couleur = 2;
					//printf("b->au_devant = %d\n",b->au_devant);
		...
		if( au_but( agy ) ) {
			...
			if( a != NULL ) {
				...
				//on se fait accrocher par le joint mandy de l'agent qui nous precede
                         	dJointAttach( a->avant, a->bodyID, agy->bodyID );

                                //ma vitesse mise à zéro pour éviter les bugs...;)
				dBodySetLinearVel( agy->bodyID, 0, 0, 0);

				//reglage des paramètres
                                dJointSetHingeAnchor( a->avant, pos_agent[0],  pos_agent[1], pos_agent[2] );

				dJointSetHingeAxis( a->avant, 1, 0, 0);
	...
	case 6 :
		...
		if( bobo != (dBodyID) 0 ){
			...
			a = cherche_agent( bobo );
                        ...
                        //si je trouve un agent alors il passe à l'état 6
			if( a != NULL && a->etat != 6 ) {
				a->etat = 6;
				a->couleur = 6;
				a->cycle = 0;
		...
		//Si il n'y a personne qui s'attache à moi
			...
			if( a == NULL) {
					...
					dJointAttach( agy->patte, 0, 0 );
                                        ...
                                        //quel objet tiens-je
					bobo = dJointGetBody(agy->avant, 1);
					...
					if( a != NULL && a->au_dessus == 0 ) {
						...
						//on signifie le fait qu'on est détaché
						agy->au_devant = 0;
...
Agent *suivant_de_la_chaine( Agent *agy ) {
	...
	//je regarde quel agent est relié par le joint mandy
        suivant = dJointGetBody(agy->avant, 1);
	autre = dJointGetBody(agy->avant, 0);
