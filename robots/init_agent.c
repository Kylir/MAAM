...
Agent *initAgent ( Agent *agy, int place, dReal x, dReal y, dReal z, dWorldID world, dSpaceID space ) {
	...
	//le joint d'aggripage avant
	agy->avant = dJointCreateHinge(world, 0);

        //le joint d'aggripage arriere
	agy->arriere = dJointCreateHinge(world, 0);

	//le joint d'aggripage gauche
	agy->gauche = dJointCreateHinge(world, 0);

	//le joint d'aggripage droite
	agy->droite = dJointCreateHinge(world, 0);


	//le booléen en_marche
	agy->etat = 0; //etat de départ de simulation
...
void init_groupe_agents( Agent tab_agents[] , int nb_agents, dWorldID monde, dSpaceID espace ){
	...
	int places[(int)(LARGEUR*1)][(int)(LONGUEUR*4)];
	int i,j,randx,randy;
	...
	for( i=0 ; i<nb_agents ; i++ ){
		...
		pa = initAgent ( &(tab_agents[i]), i, ajustx+((double)randx/4), ajusty+((double)randy/8)+0.0001, ajustz , monde, espace );//
		tab_agents[i] = *pa;
	}
}
...
void init_ligne_agents( Agent tab_agents[] , int nb_agents, dWorldID monde, dSpaceID espace ){
	...
	//pour tous les agent
	for( i=0 ; i<nb_agents ; i++ ){
		pa = initAgent ( &(tab_agents[i]), i,  x_agent , y_agent , z_agent , monde, espace );
		tab_agents[i] = *pa;
		x_agent += x_esp;
	}
}

/* Obsolete : il faut rajouter la place en arg 2 !!!
